...
void Forest::showProgress(std::string operation, size_t max_progress) {
  ...
  while (progress < max_progress) {
    ...
    auto PC =  round(100 * relative_progress);
    if (PC != percent) {
      percent = PC;
      emit updateProgress(percent); // operation variable contains what it's doing in the various phases
    }
